...
export const decrypted = token => async dispatch => {
  try {
    const response = await decryptedAPI(token);
    console.log('decrypted API Response :', response);

    return response;
  } catch (error) {
    console.log('decrypted API Error :', error);

    throw error;
  }
};

export const fetchSubCategories = (token, outlets_ID) => async dispatch => {
  try {
    dispatch(setSubCategoriesLoader());
    const response = await subCategorysAPI(token, outlets_ID);
    // console.log('Outlet Sub Categories Response :', response);
    dispatch(setSubCategoriesSuccess(response));
    return response;
  } catch (error) {
    dispatch(
      setSubCategoriesFailure(error.error || 'Failed to fetch sub categories'),
    );
    throw error;
  }
};
...
export const fetchArea = token => async dispatch => {
  try {
    dispatch(setAreaLoader());
    const response = await areaAPI(token);
